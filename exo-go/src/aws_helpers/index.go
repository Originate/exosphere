...
func createRemoteState(currSession *session.Session, config *aws.Config, bucketName string) error {
	...
	if !hasBucket(buckets.Buckets, bucketName) {
...
func createLockTable(currSession *session.Session, config *aws.Config, tableName string) error {
	...
	if !hasTable(tables.TableNames, tableName) {
...
func hasBucket(buckets []*s3.Bucket, bucketName string) bool {
...
func hasTable(tables []*string, tableName string) bool {
