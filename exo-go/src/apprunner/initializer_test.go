...
var _ = Describe("Initializer", func() {
	It("should create a docker.compose.yml", func() {
		...
		allServices := stringtools.JoinStringSlices(internalServices, externalServices, internalDependencies, externalDependencies)
		...
		initializer, err := apprunner.NewInitializer(appConfig, mockLogger, appDir, homeDir)
		Expect(err).NotTo(HaveOccurred())
		err = initializer.Initialize()
		Expect(err).NotTo(HaveOccurred())
		expectedDockerComposePath := path.Join(appDir, "tmp", "docker-compose.yml")
		Expect(ostools.DoesFileExist(expectedDockerComposePath)).To(Equal(true))
		...
		for _, serviceName := range stringtools.JoinStringSlices(internalDependencies, externalDependencies, externalServices) {
		...
		for _, serviceName := range append(internalServices, externalServices...) {
			exists := stringtools.DoesStringArrayContain(dockerCompose.Services[serviceName].DependsOn, "exocom0.22.1")
			Expect(exists).To(Equal(true))
		}

		By("should include external dependencies as dependencies")
		exists := stringtools.DoesStringArrayContain(dockerCompose.Services["todo-service"].DependsOn, "mongo3.4.0")
		Expect(exists).To(Equal(true))
