...
var addTemplateCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if osHelpers.DirectoryExists(templateDir) {
			...
			if err := exotemplate.Add(gitURL, templateName, templateDir); err != nil {
				panic(err)
			}
		}
		fmt.Println("\ndone")
...
var fetchTemplatesCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if err := exotemplate.Fetch(); err != nil {
			panic(err)
		}
		fmt.Println("\ndone")
	},
...
var removeTemplateCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if !osHelpers.DirectoryExists(templateDir) {
			...
			if err := exotemplate.Remove(templateName, templateDir); err != nil {
				panic(err)
			}
			fmt.Println("\ndone")
		}
