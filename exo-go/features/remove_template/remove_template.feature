Feature: removing existing templates
  ...
  - run "exo remove-template <name>" to remove an existing service template from the application codebase
  ...
  Scenario: removing an existing service template
    ...
    When running "exo remove-template boilr-spark" in my application directory
    Then my git repository does not have any submodules


  Scenario: removing a non-existing service template
    When starting "exo remove-template non-existing" in my application directory
