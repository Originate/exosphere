...
func TemplateFeatureContext(s *godog.Suite) {
	s.Step(`^my application is a Git repository$`, func() error {
		if _, err := runtools.Run(appDir, "git", "init"); err != nil {
	...
	s.Step(`^my application has the templates:$`, func(table *gherkin.DataTable) error {
		for _, row := range table.Rows[1:] {
			...
			if _, err := runtools.Run(appDir, "exo", "template", "add", templateName, gitURL); err != nil {
	...
	s.Step(`^my application contains the directory "([^"]*)"`, func(directory string) error {
		...
		if !ostools.DoesDirectoryExist(dirPath) {
			return fmt.Errorf("%s does not exist", directory)
		}
		if ostools.IsEmptyDirectory(directory) {
	...
	s.Step(`^my git repository has a submodule "([^"]*)" with remote "([^"]*)"$`, func(submodulePath, gitURL string) error {
		if childOutput, err := runtools.Run(appDir, "git", "config", "--file", ".gitmodules", "--get-regexp", "path"); err != nil {
		...
		if childOutput, err := runtools.Run(appDir, "git", "config", "--file", ".gitmodules", "--get-regexp", "url"); err != nil {
	...
	s.Step(`^my git repository does not have any submodules$`, func() error {
		if !ostools.IsEmptyDirectory(path.Join(appDir, ".exosphere")) || !ostools.IsEmptyFile(path.Join(appDir, ".gitmodules")) || !ostools.IsEmptyDirectory(path.Join(appDir, ".git", "modules")) {
