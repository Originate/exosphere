...
module.exports = ->
  ...
  @Given /^an instance of this service$/, (done) ->
    port-reservation.get-port N (service-port) ~>
      ...
      @process = new ExoService service-name: service-config.name, exocom-port: @exocom.port, exorelay-port: service-port
      @process.listen!
  ...
  @When /^receiving the "([^"]*)" command$/, (commandName) ->
    @exocom.reset!
    @exocom.send-message service: service-config.name, name: command-name
