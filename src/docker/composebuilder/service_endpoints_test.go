...
var _ = Describe("ServiceEndpoints", func() {
	...
	It("compiles the proper local development endpoints", func() {
		...
		s := composebuilder.NewServiceEndpoint(appConfig.Name, serviceRole, serviceConfigs[serviceRole], portReservation, buildMode)
	...
	It("compiles the proper production development endpoints", func() {
		portReservation := types.NewPortReservation()
		buildMode := composebuilder.BuildMode{
			Type:        composebuilder.BuildModeTypeLocal,
			Mount:       true,
