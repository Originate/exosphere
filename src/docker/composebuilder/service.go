...
func GetServiceDockerConfigs(appConfig types.AppConfig, serviceConfig types.ServiceConfig, serviceData types.ServiceData, role string, appDir string, homeDir string, mode BuildMode, serviceEndpoints map[string]*ServiceEndpoints) (types.DockerConfigs, error) {
	switch {
	...
	case mode.Environment == BuildModeEnvironmentTest:
		...
		return NewDevelopmentDockerComposeBuilder(appConfig, serviceConfig, serviceData, testRole, appDir, homeDir, mode, serviceEndpoints).getServiceDockerConfigs()
	default:
		return NewDevelopmentDockerComposeBuilder(appConfig, serviceConfig, serviceData, role, appDir, homeDir, mode, serviceEndpoints).getServiceDockerConfigs()
	}
}
