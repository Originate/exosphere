...
var _ = Describe("Initializer", func() {
	...
	It("should create a docker-compose.yml for production", func() {
		...
		initializer, err := application.NewInitializer(appConfig, mockLogger.GetLogChannel(""), "exo-run", appDir, homeDir, dockerComposeProjectName, composebuilder.BuildModeDeployProduction)
		Expect(err).NotTo(HaveOccurred())
	...
	It("should create a docker-compose.yml for development", func() {
		...
		mockLogger := application.NewLogger([]string{}, []string{}, ioutil.Discard)
		dockerComposeProjectName := composebuilder.GetDockerComposeProjectName(appDir)
		initializer, err := application.NewInitializer(appConfig, mockLogger.GetLogChannel(""), "exo-run", appDir, homeDir, dockerComposeProjectName, composebuilder.BuildModeLocalDevelopment)
		Expect(err).NotTo(HaveOccurred())
