Feature: scaffolding an ExoService written in ES6
  ...
  Scenario: calling with all command line arguments
    ...
    When running "exo-add service --service-role=users --service-type=users --author=test-author --template-name=exoservice-es6 --model=user --description=testing  --protection-level=public" in this application's directory
